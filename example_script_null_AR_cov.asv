...
for i_rand=1:length(random_types)
    ...
    figure(1);%figure('position',[200,200,1400,200])
    subplot(3,1,i_rand)
    ph = plot(1:t,rssr,'color',ones(1,3)*0.65);
    hold on;
    qh = plot(idxpeak,rss(idxpeak),'g*',1:t,rss,'k','linewidth',.2);
    xlim([1,t])
    ...
    figure(2)
    subplot(2,2,i_rand+1);
    %axes('outerposition',[0.15,0.15,0.85,0.85]);
    imagesc(mat(idxsort,idxsort),[-3,3]);colormap(bluewhitered)
    axis square
    axis off
...
figure(2)
subplot(2,2,1);
imagesc(FC(idxsort,idxsort),[-3,3]);colormap(bluewhitered)
axis square
axis off
hold on;
idx = find(diff(lab(idxsort)));
